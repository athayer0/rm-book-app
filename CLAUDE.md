...
Bottom tab navigator (`@react-navigation/bottom-tabs`) with 4 tabs: Home, Calendar, People, Settings. Defined in `src/navigation.tsx`. Goals have no screen or tab of their own — they surface on `HomeScreen` through two `GoalGrid`s (one per grain, in one card) plus three sheets: `EditGoalsModal` (definitions), `GoalsModal` (counts and targets), and `GoalGraphModal` (six periods of history).
...
Goals are weekly counts tracked against a target (the `goal` field on `GoalDefinition`). Definitions are stored in `useWeeklyGoals`. Completing a calendar event auto-increments whatever goal its **event type** is linked to — `getGoalContribution()` in `eventUtils.ts` applies that link and contains no per-type branch of its own. The link is four fields on `EventTypeDefinition`: `goalId`, `goalMode` (`count` / `hours` / `quantity`), plus `lateGoalId` + `goalSplitTime` for a **morning/evening pair**, where completions from the cutoff onward count toward the second goal (a completion *at* the cutoff counts late). `BUILTIN_GOAL_LINKS` in `eventTypeDefaults.ts` seeds the shipped wiring as each type's default — prayer's morning/nightly pair among them, which used to be a hardcoded `if (event.type === 'prayer')`.

**A set `goalSplitTime` is what makes a link a pair** — the flag as well as the time. In the UI that's the Goal Count Type control's fourth option, **Day/Night**, which is not a fourth `goalMode` but `'count'` plus a cutoff, so no reader has to translate it back into one. Picking it relabels Linked Goal to Morning Goal and reveals the Evening Goal and Switches At rows; picking any other option takes the cutoff *and* the evening goal back off, since a goal still claimed by a type that no longer counts it is a goal that can't be deleted for a reason nothing on screen states. A Day/Night type whose evening goal isn't chosen yet counts everything toward `goalId`.

**No goal id is written into app code.** Delete the prayer type, or either prayer goal, and the pair can be rebuilt from `EventTypesModal` — as can any other morning/evening pair, which is what the split fields exist for. A goal cannot be deleted while a type links to it in *either* slot (`goalInUse` in `EditGoalsModal`), so a contribution can never target a goal that isn't there.

**Unlinking stores `null`, never `undefined`.** A built-in's stored fields merge *over* its shipped default, so an absent `goalId` re-seeds the link — and `undefined` can't be the thing that says "unlinked", because `JSON.stringify` drops the key on the way to AsyncStorage. `fromRow` normally skips server nulls; `NULLABLE_COLUMNS` in `rowMappers.ts` exempts these three columns so an unlink survives a pull too. The three fields also seed **as a unit** (`mergeWithDefaults`): a definition carrying its own `goalId` gets no seeded default at all, or a type someone relinked to a single goal would silently acquire the shipped default's evening half.
...
Week keys are strings like `"2025-W21"` generated by `getWeekKey()` in `src/utils/dateUtils.ts`. Month keys are `"2025-08"`, from `getMonthKey()`.

**Every goal is tracked at one or both grains, and no UI component is written twice for it.** `src/utils/goalGrain.ts` holds the `GRAIN` table — per grain, the tab labels, the visibility flag (`visible` / `monthlyVisible`), the key-by-offset function, and the two label formatters. `GoalPeriodList` (the counts-and-targets list) and `GoalGraph` (the six-period chart) each take a `grain` prop and read the rest from that table; the sheets pass in the data accessors, which are the one thing that can't live there because they come off `useWeeklyGoals` / `useMonthlyGoals`. Before this there was a weekly file and a near-identical monthly copy of each, and they had already drifted. **Do not add a monthly variant of a goal component — add a `GRAIN` entry's worth of difference instead.**

A goal's grain is the Goal Period segmented control in `EditGoalsModal` (Weekly / Monthly / Both), which writes the `visible`/`monthlyVisible` pair. There is no "hidden from both" state and no separate monthly-visibility screen. `MAX_VISIBLE_GOALS` caps each grid independently, so a segment is disabled only when it would push a grid the goal isn't already on past the cap.
